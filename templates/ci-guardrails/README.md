...
The **pattern** is Layer B.2 (vendor-agnostic, stack-agnostic). The concrete YAML here is a GitHub Actions **instance** (Layer C.2). Each header documents the equivalent for GitLab / CircleCI / Buildkite / Jenkins so adopters instantiate their own C.2.
