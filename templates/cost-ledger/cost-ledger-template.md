...
- [ ] Bootstrap CI guardrails active (concurrency-cancel, change-gating,
      aggregation-gate, draft-skip, and dependency-cache) — NOT retrofit
- [ ] LLM-CI levers active when paid-LLM endpoint is called (prompt-caching,
      model-tier, diff-only context, trigger-set minimalism, and LLM-aware
...
- Cost-ledger that tracks only tokens (infra-cost and LLM-API blindness)
- Cost-ledger that tracks tokens and infra but NOT LLM-API-in-CI (the most
